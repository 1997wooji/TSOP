...
public class MusicViewDAO {
	...
	/**MusicVO 객체를 받아 데이터를 저장하는 메소드 MusicVO에 fileId와 memberId가 없어서 만들지 못한다.*/
	...
	public int deleteLikeMusic(String memberId, int musicId){
		MusicDAO musicDAO = new MusicDAO();
		int res = -1;
		res = musicDAO.deleteLikeMusic(memberId, musicId);
		return res;
	}
		...
		try{
			...
			rs = psmt.executeQuery();
				if(rs.next()){
					...
					music = new MusicVO(rs.getInt(1), rs.getString(2), rs.getString(3), rs.getString(4), rs.getString(5), 
							rs.getInt(6), rs.getDate(7).toString(), rs.getInt(8), rs.getString(9), rs.getString(10),
							rs.getString(11), rs.getInt(12), false,false );
					if(rs.getString(13).equals("1")){
						music.setVisibility(true);
					}
					if(rs.getString(14).equals("1")){
						music.setDownAccept(true);
					}
				}
	...
	public Object[] searchTitleMusic(String musicKey){
		String selectTag = "SELECT * FROM music_view WHERE music_title like'%"+musicKey+"%' ";
		List<MusicVO> list = new ArrayList<MusicVO>();
		Connection con = ConnectDB.connect();
		PreparedStatement psmt= null;
		ResultSet rs = null;
	...
	public Object[] searchTopLikeMusic(){
		String select = "SELECT * FROM music_view ORDER BY like_cnt desc";
		List<MusicVO> list = new ArrayList<MusicVO>();
		MusicVO music = null;
		Connection con = ConnectDB.connect();
		PreparedStatement psmt = null;
	...
	public Object[] searchTopPlayMusic(){
		String select = "SELECT music_title, like_cnt FROM music_view GROUP BY(music_id) ORDER BY music_play_count desc";
		List<MusicVO> list = new ArrayList<MusicVO>();
		MusicVO music = null;
		Connection con = ConnectDB.connect();
		PreparedStatement psmt = null;
	...
	/**musicId를 받아서 좋아요를 누른 사람 목록을 반환하는 메소드*/
	...
	public int searchMusicFileId(int musicId){
		...
		MusicDAO musicDAO = new MusicDAO();
		int fileId=musicDAO.searchMusicFileId(musicId);	
		return fileId;
